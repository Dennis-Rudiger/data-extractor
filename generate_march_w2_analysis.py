...
week2_data = merge_reps(raw_data)
...
reps_data = get_rep_data(week2_data)
...
def create_rep_bar_chart(output_dir="charts/march_w2"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
...
def create_margin_comparison(output_dir="charts/march_w2"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
...
def create_stacked_bar(output_dir="charts/march_w2"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
    fig, ax = plt.subplots(figsize=(10, 5))
...
def create_radar_chart(output_dir="charts/march_w2"):
    os.makedirs(output_dir, exist_ok=True)
...
def create_rep_donut(rep_name, rep_data, output_dir="charts/march_w2"):
...
def create_rep_sales_profit_bar(rep_name, rep_data, output_dir="charts/march_w2"):
...
print(f"  {len(chart_files)} summary charts generated in charts/march_w2/")
...
# ========== EXCEL REPORT ==========
def generate_excel():
    output_fn = f"sales_analysis_march_w2.xlsx"
    wb = Workbook()
...
# ========== WORD REPORT ==========
def generate_word():
    output_fn = f"sales_analysis_march_w2.docx"
    doc = DocxDocument()
...
# ========== JSON REPORT ==========
def generate_json():
    output_fn = f"sales_analysis_march_w2.json"
