"""
Update MOQ Analysis for Q1 2026
======================================
Reads: stock_movement_karen_q1.json  (from INVENTORY MOVEMENT FEBRUARY.pdf)
       item_category_map.json   (from INVENTORY COUNT feb.pdf)
       inventory.json           (master product catalog with pricing)

Outputs:
  - inventory.json  (updated with q1_qty_out, q1_closing, daily_consumption, moq)
  - karen_average_moq.json (full MOQ analysis for February)

Period: January 5 - March 31, 2026 (24 working days)
...
PERIOD_SHORT = "Q1 2026"

# ===== LOAD DATA =====
print("=" * 60)
print("MOQ UPDATE — Q1 2026")
print("=" * 60)

with open('stock_movement_karen_q1.json', 'r') as f:
    q1_data = json.load(f)
...
q1_items = q1_data['items']
print(f"Loaded {len(q1_items)} items from Q1 movement data")
print(f"Loaded {len(inventory)} categories from inventory.json")

# ===== BUILD LOOKUPS =====
q1_lookup = {}
for item in q1_items:
    q1_lookup[item['code']] = item
...
for cat in inventory:
    ...
    for prod in cat.get('products', []):
        ...
        if code:
            ...
            code_to_price[code] = prod.get('buying_price', prod.get('selling_price', 0))
...
with open('karen_average_moq.json', 'w') as f:
    json.dump(analysis, f, indent=2)
print(f"MOQ analysis saved to karen_average_moq.json")
...
print("\nDone! Now run: py generate_moq_report_karen.py")
