...
def load_data():
    with open('moq_analysis_karen.json', 'r') as f:
        return json.load(f)
...
def build_insights(data):
    ...
    insights = {
        'executive_summary': (
            f"In February 2026, BOMAS Hardware (Karen Branch) tracked {s['total_items']:,} SKUs across 23 categories "
        ...
        'demand_concentration': (
            ...
            f"(~1,168 units/day) can result in substantial lost revenue."
        ),
        'category_value': (
        ...
        'moq_importance': (
            ...
            "average consumption (equivalent to one week's supply), BOMAS Karen ensures: (1) sufficient stock "
        ...
        'recommendations': [
            ...
            "Negotiate better terms with Cement and Steel suppliers given the high weekly spend (combined KES 10.8M/wk).",
        ],
    }
    return insights
...
def build_jan_feb_comparison():
    ...
    import os
    if not os.path.exists('stock_movement_karen_jan.json') or not os.path.exists('stock_movement_karen_feb.json'):
        return None

    with open('stock_movement_karen_jan.json') as f:
        jan = json.load(f)
    with open('stock_movement_karen_feb.json') as f:
        feb = json.load(f)
...
class MOQReport:
    def __init__(self, filename='moq_report_karen.pdf'):
    ...
    def header_footer(self, canvas, doc):
        ...
        canvas.drawString(self.margin, self.page_height - 0.33 * inch,
                          'BOMAS HARDWARE KAREN — MOQ REPORT  |  FEBRUARY 2026  |  24 Working Days  |  Formula: 6 × Daily Avg')
        ...
        canvas.drawRightString(self.page_width - self.margin, 0.22 * inch, 'Confidential — BOMAS Hardware Store (Karen Branch)')
        canvas.restoreState()
...
class MOQReportWord:
    ...
    def __init__(self, filename='moq_report_karen.docx'):
        self.filename = filename
    ...
    def generate(self):
        ...
        run = title.add_run('BOMAS HARDWARE KAREN — MOQ REPORT')
        run.font.size = Pt(16)
        run.font.bold = True
        ...
        run = footer.add_run(f'Generated: {datetime.now().strftime("%d %b %Y %H:%M")}  |  Confidential — BOMAS Hardware Store (Karen Branch)')
        run.font.size = Pt(7)
        run.font.color.rgb = RGBColor(*self.hex_to_rgb(GREY_LIGHT))
...
if __name__ == '__main__':
    report = MOQReport('moq_report_karen.pdf')
    report.generate()

    word_report = MOQReportWord('moq_report_karen.docx')
