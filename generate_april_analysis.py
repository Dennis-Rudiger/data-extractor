"""
Generate comprehensive April sales analysis reports.
Period: April 1 - April 30, 2026 (6 working days)
    ...
    for cat, reps in data["categories"].items():
        ...
        for rep, vals in reps.items():
            if rep.lower() == "unknown":
                continue
            # Exclude zero-sales anonymous walk-ins from ranking
            if rep.upper().startswith("WALK IN"):
                continue
            if rep == "Magdalene":
                target = "Betha Odumo"
            elif rep == "Esther":
...
week1_data = merge_reps(raw_data)
...
reps_data = get_rep_data(week1_data)
...
# ========== CHART GENERATION ==========
def create_category_pie(output_dir="charts/april"):
...
def create_rep_bar_chart(output_dir="charts/april"):
    os.makedirs(output_dir, exist_ok=True)
...
def create_margin_comparison(output_dir="charts/april"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
...
def create_target_progress(output_dir="charts/april"):
    os.makedirs(output_dir, exist_ok=True)
    ...
    for ax, (label, actual, target) in zip(axes, items):
        ...
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
    fig.suptitle(f'Monthly Target Progress in {MONTH} {YEAR} (Expected: 100%)', fontweight='bold', fontsize=11)
    plt.tight_layout()
...
def create_stacked_bar(output_dir="charts/april"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
    fig, ax = plt.subplots(figsize=(10, 5))
...
def create_radar_chart(output_dir="charts/april"):
    os.makedirs(output_dir, exist_ok=True)
...
def create_rep_donut(rep_name, rep_data, output_dir="charts/april"):
...
def create_rep_sales_profit_bar(rep_name, rep_data, output_dir="charts/april"):
...
print(f"  {len(chart_files)} summary charts generated in charts/april/")
...
# ========== PDF REPORT ==========
def generate_pdf():
    output_fn = f"sales_analysis_april.pdf"
    ...
    elements.append(Paragraph(f"The {top_cat} department remains the dominant revenue driver, accounting for {top_cat_share:.1f}% of total April sales (KES {top_cat_sales:,.0f}). However, margin analysis reveals that {highest_margin_cat} operates at the highest profitability rate ({highest_margin_cat_margin:.1f}%). Strategic focus should be placed on pushing higher-margin {highest_margin_cat} items in May to lift the overall {overall_margin:.1f}% store margin.", body_s))
    elements.append(Spacer(1, 10))
      
    elements.append(Paragraph("3. Sales Team Productivity", subheading_s))
    elements.append(Paragraph(f"Sales concentration is notably high among the top performers. {top_rep} led the month with KES {top_rep_sales:,.0f} in sales, followed closely by {second_rep} (KES {second_rep_sales:,.0f}). Combined, these top two representatives accounted for {top_2_share:.1f}% of the store's total April volume. To mitigate key-person dependency risks, training and mentorship programs should be implemented for the remaining {len(reps_data)-2} active representatives.", body_s))
    elements.append(Spacer(1, 10))
      
    elements.append(Paragraph("4. May Operational Recommendations", subheading_s))
    ...
    for rank, (rep_name, rep_data) in enumerate(sorted_reps, 1):
        rep_data_rows.append([
            ...
            f"{rep_data['total_sales'] / total_sales * 100:.1f}%"
        ])
    
    t = Table(rep_data_rows, colWidths=[0.4*inch, 1.2*inch, 1.3*inch, 1.1*inch, 1.0*inch, 0.7*inch, 0.7*inch, 0.7*inch])
...
# ========== EXCEL REPORT ==========
def generate_excel():
    output_fn = f"sales_analysis_april.xlsx"
    wb = Workbook()
    ...
    headers = ["Category", "Monthly Target", "Week 1 Actual", "% Achieved", "Expected %", "Pace"]
    ws.append(headers)
...
# ========== WORD REPORT ==========
def generate_word():
    output_fn = f"sales_analysis_april.docx"
    doc = DocxDocument()
    ...
    run = p.add_run(f"After Week 1, the branch has achieved {target_pct:.1f}% of the monthly target (KES {MONTHLY_TARGETS['OVERALL']/1000000:.0f}M). This puts us {pace_text} the expected 25% pace.")
    run.font.size = Pt(10)
    
    table = doc.add_table(rows=len(CAT_ORDER) + 2, cols=5)
    table.style = 'Table Grid'
    headers = ["Category", "Monthly Target", "Week 1 Actual", "% Achieved", "Pace"]
    ...
    # Add charts
    doc.add_paragraph()
    for chart_key in ['rep_bar', 'margin', 'stacked', 'category_pie', 'radar']:
    ...
    # April Strategic Insights (Word)
    doc.add_page_break()
    p = doc.add_paragraph()
    run = p.add_run("April 2026 STRATEGIC BUSINESS INSIGHTS")
    run.bold = True
    run.font.size = Pt(14)
    run.font.color.rgb = RGBColor(0x2c, 0x3e, 0x50)

    p = doc.add_paragraph()
    run = p.add_run("1. Overall Monthly Performance")
    run.bold = True
    doc.add_paragraph(f"BOMAS Hardware Store generated KES {total_sales:,.0f} in revenue and KES {total_profit:,.0f} in gross profit during the first {WORKING_DAYS} working days of April 2026. This translates to an average daily revenue of KES {total_sales/WORKING_DAYS:,.0f} and an average daily profit of KES {total_profit/WORKING_DAYS:,.0f}. The overall profit margin stabilized at {overall_margin:.1f}%.")
    
    p = doc.add_paragraph()
    run = p.add_run("2. Departmental Contribution & Margin Analysis")
    run.bold = True
    doc.add_paragraph(f"The {top_cat} department remains the dominant revenue driver, accounting for {top_cat_share:.1f}% of total April sales (KES {top_cat_sales:,.0f}). However, margin analysis reveals that {highest_margin_cat} operates at the highest profitability rate ({highest_margin_cat_margin:.1f}%). Strategic focus should be placed on pushing higher-margin {highest_margin_cat} items in May to lift the overall {overall_margin:.1f}% store margin.")
    
    p = doc.add_paragraph()
    run = p.add_run("3. Sales Team Productivity")
    run.bold = True
    doc.add_paragraph(f"Sales concentration is notably high among the top performers. {top_rep} led the month with KES {top_rep_sales:,.0f} in sales, followed closely by {second_rep} (KES {second_rep_sales:,.0f}). Combined, these top two representatives accounted for {top_2_share:.1f}% of the store's total April volume. To mitigate key-person dependency risks, training and mentorship programs should be implemented for the remaining {len(reps_data)-2} active representatives.")
    
    p = doc.add_paragraph()
    run = p.add_run("4. May Operational Recommendations")
    run.bold = True
    ...
    doc.add_paragraph("- Align Minimum Order Quantities (MOQs) with April daily consumption averages to ensure optimal working capital allocation.")
...
# ========== JSON REPORT ==========
def generate_json():
    output_fn = f"sales_analysis_april.json"
