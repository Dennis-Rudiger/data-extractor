"""
Generate comprehensive March Week 1 sales analysis reports.
...
# ========== CHART GENERATION ==========
def create_category_pie(output_dir="charts/march_w1"):
...
def create_rep_bar_chart(output_dir="charts/march_w1"):
    os.makedirs(output_dir, exist_ok=True)
...
def create_margin_comparison(output_dir="charts/march_w1"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
...
def create_target_progress(output_dir="charts/march_w1"):
    os.makedirs(output_dir, exist_ok=True)
...
def create_stacked_bar(output_dir="charts/march_w1"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
    fig, ax = plt.subplots(figsize=(10, 5))
...
def create_radar_chart(output_dir="charts/march_w1"):
    os.makedirs(output_dir, exist_ok=True)
...
def create_rep_donut(rep_name, rep_data, output_dir="charts/march_w1"):
...
def create_rep_sales_profit_bar(rep_name, rep_data, output_dir="charts/march_w1"):
...
print(f"  {len(chart_files)} summary charts generated in charts/march_w1/")
...
# ========== PDF REPORT ==========
def generate_pdf():
    output_fn = f"sales_analysis_march_w1.pdf"
...
# ========== EXCEL REPORT ==========
def generate_excel():
    output_fn = f"sales_analysis_march_w1.xlsx"
    wb = Workbook()
...
# ========== WORD REPORT ==========
def generate_word():
    output_fn = f"sales_analysis_march_w1.docx"
    doc = DocxDocument()
...
# ========== JSON REPORT ==========
def generate_json():
    output_fn = f"sales_analysis_march_w1.json"
