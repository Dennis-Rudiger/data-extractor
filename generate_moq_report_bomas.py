...
def load_data():
    with open('bomas_average_moq.json', 'r') as f:
        return json.load(f)
...
def build_insights(data):
    ...
    insights = {
        'executive_summary': (
            f"In Q1 (Jan-Mar) 2026, BOMAS Hardware tracked {s['total_items']:,} SKUs across 23 categories "
            f"with total outward movement of {total_qty:,.0f} units over 74 working days. "
...
class MOQReport:
    def __init__(self, filename='bomas_average_moq.pdf'):
    ...
    def header_footer(self, canvas, doc):
        ...
        canvas.drawString(self.margin, self.page_height - 0.33 * inch,
                          'BOMAS HARDWARE — MOQ REPORT  |  Q1 2026  |  74 Working Days  |  Formula: 6 × Daily Avg')
    ...
    def generate(self):
        ...
        elements.append(Paragraph(
            'Period: Jan 5 - Mar 31, 2026  •  74 Working Days (12 weeks × 6 days)  •  '
            'Daily Avg = Qty Out ÷ 74  •  MOQ = Daily Avg × 6',
        ...
        elements.append(Paragraph(
            'Based on the Q1 2026 movement data, the following actions are recommended:',
            self.styles['note']))
        elements.append(Spacer(1, 4))
        ...
        if comparison:
            elements.append(PageBreak())
            elements.append(Paragraph('Q1 2026 TREND ANALYSIS', self.styles['title']))
            elements.append(Paragraph(
                f'{comparison["jan_period"]}  →  {comparison["feb_period"]}  |  '
                f'Total: 74 working days  |  {comparison["common_items"]:,} common items compared',
                self.styles['subtitle']))
            elements.append(Spacer(1, 6))
        ...
        elements.append(Paragraph(
            'Items ranked by total quantity moved out in Q1 2026. '
        ...
        elements.append(Paragraph(
            'Items that received stock but recorded no outward movement during Q1. '
...
class MOQReportWord:
    ...
    def __init__(self, filename='bomas_average_moq.docx'):
        self.filename = filename
    ...
    def generate(self):
        ...
        run = note.add_run('Based on the Q1 2026 movement data, the following actions are recommended:')
        run.font.size = Pt(8.5)
        ...
        if comparison:
            ...
            heading = doc.add_paragraph()
            heading.alignment = WD_ALIGN_PARAGRAPH.CENTER
            run = heading.add_run('Q1 2026 TREND ANALYSIS')
            run.font.size = Pt(14)
            run.font.bold = True
            run.font.color.rgb = RGBColor(*self.hex_to_rgb(NAVY))
...
if __name__ == '__main__':
    report = MOQReport('bomas_average_moq.pdf')
    report.generate()

    word_report = MOQReportWord('bomas_average_moq.docx')
