...
def load_data():
    with open('moq_analysis_karen_feb_apr.json', 'r') as f:
        return json.load(f)
...
def build_insights(data):
    ...
    insights = {
        'executive_summary': (
            ...
            f"with total outward movement of {total_qty:,.0f} units over 76 working days. "
...
class MOQReport:
    def __init__(self, filename='moq_report_karen_feb_apr.pdf'):
    ...
    def header_footer(self, canvas, doc):
        ...
        canvas.drawString(self.margin, self.page_height - 0.33 * inch,
                          'KAREN HARDWARE — MOQ REPORT  |  JAN-FEB 2026  |  76 Working Days  |  Formula: 6 × Daily Avg')
        ...
        canvas.drawRightString(self.page_width - self.margin, 0.22 * inch, 'Confidential — Karen Hardware Store')
        canvas.restoreState()
    ...
    def generate(self):
        ...
        elements.append(Paragraph(
            'Period: Feb 1 - Apr 30, 2026  •  76 Working Days (13 weeks × 6 days)  •  '
            'Daily Avg = Qty Out ÷ 76  •  MOQ = Daily Avg × 6',
        ...
        elements.append(Paragraph(
            'Based on the Feb-Apr 2026 movement data, the following actions are recommended:',
            self.styles['note']))
        elements.append(Spacer(1, 4))
        ...
        if comparison:
            ...
            elements.append(Paragraph(
                ...
                f'Both months: 76 working days  |  {comparison["common_items"]:,} common items compared',
                self.styles['subtitle']))
            elements.append(Spacer(1, 6))
        ...
        elements.append(Paragraph(
            'Items ranked by total quantity moved out in Feb-Apr 2026. '
...
class MOQReportWord:
    ...
    def __init__(self, filename='moq_report_karen_feb_apr.docx'):
        self.filename = filename
    ...
    def generate(self):
        ...
        run = title.add_run('KAREN HARDWARE — MOQ REPORT')
        run.font.size = Pt(16)
        run.font.bold = True
        ...
        run = note.add_run('Based on the Feb-Apr 2026 movement data, the following actions are recommended:')
        run.font.size = Pt(8.5)
        ...
        run = footer.add_run(f'Generated: {datetime.now().strftime("%d %b %Y %H:%M")}  |  Confidential — Karen Hardware Store')
        run.font.size = Pt(7)
        run.font.color.rgb = RGBColor(*self.hex_to_rgb(GREY_LIGHT))
...
if __name__ == '__main__':
    report = MOQReport('moq_report_karen_feb_apr.pdf')
    report.generate()

    word_report = MOQReportWord('moq_report_karen_feb_apr.docx')
