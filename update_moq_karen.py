"""
MOQ Analysis for KAREN Branch — February 2026
==============================================
Reads: stock_movement_karen_feb.json  (from KAREN INVENTORY MOVEMENT FEB.pdf)
       item_category_map.json         (category map for categorisation)
       inventory.json                 (master product catalog with pricing)

Outputs:
  - moq_analysis_karen.json (full MOQ analysis for Karen branch, February)
...
# ===== LOAD DATA =====
print("=" * 60)
print(f"MOQ ANALYSIS — {BRANCH} BRANCH — {PERIOD_SHORT.upper()}")
print("=" * 60)

with open('stock_movement_karen_feb.json', 'r') as f:
...
print(f"Loaded {len(feb_items)} items from Karen February movement data")
...
for cat in inventory:
    ...
    for prod in cat.get('products', []):
        ...
        if code:
            ...
            code_to_price[code] = prod.get('selling_price', 0)
...
with open('moq_analysis_karen.json', 'w') as f:
    json.dump(analysis, f, indent=2)
print(f"MOQ analysis saved to moq_analysis_karen.json")

# ===== PRINT SUMMARY =====
print("\n" + "=" * 70)
print(f"MOQ ANALYSIS — {BRANCH} BRANCH — {PERIOD_SHORT.upper()} ({WORKING_DAYS} Working Days)")
print("=" * 70)
...
print(f"\nDone! Karen MOQ analysis saved to moq_analysis_karen.json")
print("Run: py generate_moq_report_karen.py  to generate PDF/Word reports")
