...
# ========== CONFIGURATION ==========
PERIOD = "January 5 - March 31, 2026"
WORKING_DAYS = 74
MONTH = "Q1"
YEAR = 2026
WEEK_NUM = "Q1"
...
# ========== LOAD DATA ==========
with open("sales_q1.json", "r") as f:
...
def create_rep_bar_chart(output_dir="charts/q1"):
    os.makedirs(output_dir, exist_ok=True)
...
def create_margin_comparison(output_dir="charts/q1"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
...
def create_stacked_bar(output_dir="charts/q1"):
    os.makedirs(output_dir, exist_ok=True)
    names = [r[0] for r in sorted_reps]
    fig, ax = plt.subplots(figsize=(10, 5))
...
def create_radar_chart(output_dir="charts/q1"):
    os.makedirs(output_dir, exist_ok=True)
...
def create_rep_donut(rep_name, rep_data, output_dir="charts/q1"):
...
def create_rep_sales_profit_bar(rep_name, rep_data, output_dir="charts/q1"):
...
# ========== EXCEL REPORT ==========
def generate_excel():
    output_fn = f"sales_analysis_q1.xlsx"
    wb = Workbook()
...
# ========== WORD REPORT ==========
def generate_word():
    output_fn = f"sales_analysis_q1.docx"
    doc = DocxDocument()
...
# ========== JSON REPORT ==========
def generate_json():
    output_fn = f"sales_analysis_q1.json"
